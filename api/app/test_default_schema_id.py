...
EXPECTED_SCHEMA_ID = "default_v1"


def test_default_schema_id_is_the_neutral_value() -> None:
    """値そのものを固定する。

    禁止語の一覧をここに持たない。それは scripts/forbidden-terms.txt が正本で、
    重複させると一覧に語を足したときにここだけ古くなる。
    """
    assert DEFAULT_LABEL_SCHEMA["schema_id"] == EXPECTED_SCHEMA_ID
