...
- **重い依存を増やさない**。行列の読み込み（`matrix.npy`）は numpy を要するため
  呼び出し側 `app/clustering.py` が担い、本モジュールには `list[list[float]]` を渡す。
  これにより CI（numpy 無し）でも本ロジックを単体テストできる（torch 教訓の踏襲）。
...
# 自動 k の上限。多すぎるクラスタは「まとめて付与」の利点を損なうため抑える。
MAX_AUTO_K = 12
...
def run_clustering(
    ...
    *,
    ...
    labels = kmeans(vectors, resolved_k, seed=seed, max_iters=max_iters)
