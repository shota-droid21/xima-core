...
1. **まとめてラベルを付ける** — 似た画像どうしが自動でまとまるので、1 枚ずつではなく
   ...
   まとまりを出す前に**「まとまりを作れるようにする」を 1 回押します**（画像を読み込んで
   埋め込みを作ります。初回はモデルの取得も走るため少し時間がかかります）
2. **学習して精度を見る** — 付けたラベルで分類モデルを学習します。CLIP の埋め込みに
   線形分類器を載せる方式で、検証精度が確認できます
3. **残りに候補を付ける** — 学習したモデルで未ラベル画像に候補を付けます。候補は
   `labels.json` にラベルとは別のフィールド（`predicted`）として記録され、ラベリング画面で
   確定したものだけがラベルになります。クラスごとのスコアは
